...
Buffer<ov::AlignedBuffer> TensorExternalData::load_external_mem_data() const {
    ...
    // m_data_length is parsed from the untrusted ONNX 'length' key and is otherwise unbounded here
    // (no backing file size to validate against). Reject implausibly large allocation requests to
    // prevent an attacker from requesting an allocation of up to 2^64-1 bytes.
    constexpr uint64_t MAX_ALLOWED_TENSOR_BYTES = static_cast<uint64_t>(1) << 40;  // 1 TiB
