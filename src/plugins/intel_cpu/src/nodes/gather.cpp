...
void Gather::createPrimitive() {
    ...
    if ((x64::mayiuse(x64::avx512_core) || x64::mayiuse(x64::avx2)) &&
        ...
        if (jitKernel) {
            ...
            if (!isDynamicNode()) {
                // Degenerate zero-work graphs would lead to division/modulo by zero below.
                // Skip the JIT precomputation; execReference handles the empty-output case gracefully.
                if (specIndicesSize == 0 || afterAxisSize == 0) {
                    return;
                }
                ...
                // Guard the divisor product against integer overflow before it is used below.
                CPU_NODE_ASSERT(betweenBatchAndAxisSize == 0 ||
                                    afterAxisSize <= UINT64_MAX / specIndicesSize / betweenBatchAndAxisSize,
                                "has integer overflow in batch index computation.");
